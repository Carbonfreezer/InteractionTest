...
[RequireComponent(typeof(RemoteControllerComponent))]
public class ItemControllerKinematic : NetworkBehaviour, ILoggable, ISteerable
{
    /// <summary>
...
#pragma warning restore CS0162
    ...
    public void Start()
    {
        ...
        RemoteSpatialManipulationController remoteController = GetComponent<RemoteControllerComponent>().Controller;
        remoteController.ControlTaken += RemoteControllerOnControlTaken;
        remoteController.ControlReleased += RemoteControllerOnControlReleased;
        remoteController.CoordinateSet += RemoteControllerOnCoordinateSet;
    }


    #region Dealing with interaction 
    private void RemoteControllerOnCoordinateSet(Vector3 position, Quaternion orientation)
    {
        if (!m_isMissionRunning)
            return;

        m_rigidBody.MovePosition(transform.parent.TransformPoint(position));
        m_rigidBody.MoveRotation(transform.parent.rotation * orientation);
    }

    private void RemoteControllerOnControlReleased()
    {
        if (!m_isMissionRunning)
            return;
    ...
    private bool RemoteControllerOnControlTaken(ulong clientid)
    {
        if (!m_isMissionRunning)
            return false;
