...
/// <summary>
/// The server side simulation of the force component.
/// </summary>
/// <seealso cref="ServerInteractionController"/>
public class ServerInteractionControllerPhysics
{
    /// <summary>
    /// The remote controller we work with.
    /// </summary>
    private readonly RemotePhysicsController m_controller;
    ...
    public ServerInteractionControllerPhysics(RemotePhysicsController controller, Transform anchor)
    {
        m_controller = controller;
        m_mainCamera = Camera.main;
        m_cameraTransform = m_mainCamera.gameObject.transform;
        m_arrayOfColliders = new List<Collider>(m_controller.gameObject.GetComponents<Collider>());
    ...
    public void Release()
    {
        if (m_wasInteracting)
        {
            RPCChanneler.Singleton.StopPhysics(m_controller, true);
            m_wasInteracting = false;
        }
    }
    ...
    public void Update()
    {
        ...
        if ((!m_wasInteracting) && Input.GetMouseButtonDown(0))
        {
            ...
            RPCChanneler.Singleton.StartPhysics(m_controller, true, pointerCoords.m_position, pointerCoords.m_rotation);
        ...
        RPCChanneler.Singleton.ProcessPhysics(m_controller, true, finalPose.m_position, finalPose.m_rotation);
    }
}